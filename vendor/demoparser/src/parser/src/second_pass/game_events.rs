...
impl<'a> SecondPassParser<'a> {
    ...
    pub fn find_extra(&self, fields: &Vec<EventField>) -> Result<Vec<EventField>, DemoParserError> {
        ...
        for field in fields {
            ...
            if let Some(Variant::I32(u)) = field.data {
                ...
                extra_fields.push(self.create_player_team_num_field(entity_id, prefix));
    ...
    pub fn create_player_team_num_field(&self, entity_id: i32, prefix: &str) -> EventField {
        let data = self.players.get(&entity_id).and_then(|player| player.team_num).map(Variant::U32);
        EventField {
            name: prefix.to_owned() + "_team_num",
            data,
        }
    }
